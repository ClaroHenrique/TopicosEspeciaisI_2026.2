# Decifra
Fonte: [https://olimpiada.ic.unicamp.br/passadas/OBI2014/fase2/programacao/](https://olimpiada.ic.unicamp.br/passadas/OBI2014/fase2/programacao/)

Dimas é um renomado investigador de roubos a antiguidades e obras de arte, que sempre é chamado para casos intrigantes que necessitam de bastante trabalho mental. Desta vez, o quadro que sumiu de um conhecido museu na França foi a **Donalisa**, do pintor *Leonardo da Silva*. 
...
* A primeira linha da entrada contém uma sequência de $26$ letras minúsculas distintas, representando a permutação inversível usada na frase criptografada. A permutação é a seguinte:
  * A letra `a` é trocada pela 1ª letra dessa sequência;
  * A letra `b` é trocada pela 2ª letra dessa sequência;
  * A letra `c` é trocada pela 3ª letra;
  * ... e assim por diante, seguindo a ordem padrão do alfabeto (`abcdefghijklmnopqrstuvwxyz`).
* A segunda linha da entrada consiste de uma frase criptografada, contendo apenas letras minúsculas.

## Saída

Seu programa deve imprimir o texto original recuperado, de acordo com a permutação fornecida.
